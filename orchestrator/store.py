...
# ============================================================
# LEMON SQUEEZY (New Primary — 5% fee)
# ============================================================

class LemonSqueezyStore:
    BASE_URL = "https://api.lemonsqueezy.com/v1"

    def __init__(self):
        self.key = LEMONSQUEEZY_API_KEY
        self.headers = {
    ...
    def _get(self, endpoint: str, params: dict = None) -> dict:
        if not self.key:
            return {"error": "No LEMONSQUEEZY_API_KEY configured"}
        resp = requests.get(f"{self.BASE_URL}/{endpoint}", headers=self.headers, params=params)
        return resp.json()
    ...
    def list_products(self) -> list[dict]:
        ...
        for p in data.get("data", []):
            ...
            products.append({
                ...
                "price": attrs.get("price_formatted", "$0.00"),
            })
        return products
    ...
    def create_product(self, name: str, price_usd: float, description: str = "") -> dict:
        ...
        if not LEMONSQUEEZY_STORE_ID:
            print("⚠️ Skipping LS Product Creation: No LEMONSQUEEZY_STORE_ID in .env")
            return {}
        ...
        data = self._post("products", payload)
        if "errors" in data:
            print(f"❌ Lemon Squeezy Error: {data['errors']}")
            return {}
        
        product_id = data["data"]["id"]
        buy_url = data["data"]["attributes"].get("buy_now_url", "")
        print(f"✅ Lemon Squeezy Product Created: {name} → {buy_url}")
    ...
    def get_revenue(self) -> dict:
        """Get revenue and sync with goal."""
        # Note: Lemon Squeezy API for revenue analysis varies by endpoint
        # For now, we return a snapshot based on recent orders
        data = self._get("orders")
        if "error" in data:
            return {"total_revenue": 0, "platform": "lemonsqueezy"}
        
        orders = data.get("data", [])
        total = sum(float(o["attributes"]["total"]) for o in orders if o["attributes"]["status"] == "paid") / 100
        count = len([o for o in orders if o["attributes"]["status"] == "paid"])
...
def get_store():
    ...
    if STORE_PLATFORM == "lemonsqueezy" and LEMONSQUEEZY_API_KEY:
        return LemonSqueezyStore()
    elif STORE_PLATFORM == "stripe" and STRIPE_SECRET_KEY:
        return StripeStore()
        ...
        print("⚠️ No store configured — set LEMONSQUEEZY_API_KEY, STRIPE_SECRET_KEY or GUMROAD_ACCESS_TOKEN in .env")
        return None
